...
class formFinal(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        # self.fields['usuario_id'] = forms.ChoiceField(
        #     choices= User.objects.all().values_list('id', 'email')
        #             if self.usuario.pertenece_a('PUEDE_CARGAR_OTROS_USUARIOS') > 0
        #             else User.objects.filter(pk=self.usuario.id).values_list('id', 'email'),
        #             widget=forms.Select, initial=self.usuario.id)
        if self.usuario.pertenece_a('PUEDE_CARGAR_OTROS_USUARIOS') >0 :
            self.fields['usuario_id'] = forms.ChoiceField(
                choices= User.objects.all().values_list('id', 'email') ,
                widget=forms.Select, initial=self.usuario.id)
        else:
            self.fields['usuario_id'] = forms.CharField()
            self.fields['usuario_id'].widget = forms.HiddenInput()
            self.fields['usuario_id'].initial = self.usuario.id
